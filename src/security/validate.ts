...
export function validateInput(body: unknown): TailorInput {
  ...
  if (b.resumeFile && typeof b.resumeFile === "object") {
    ...
    const kind =
      f.kind === "pdf" ? "pdf" : f.kind === "docx" ? "docx" : f.kind === "image" ? "image" : null;
    if (!kind) throw new ValidationError('resumeFile.kind must be "pdf", "docx", or "image".');
    ...
    const fallbackType = kind === "pdf" ? "application/pdf" : kind === "docx" ? DOCX_TYPE : "image/png";
    const mediaType = typeof f.mediaType === "string" && f.mediaType ? f.mediaType : fallbackType;
